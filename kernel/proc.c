...
int fork(void)
{
  ...
  *(np->kthread[0].trapframe) = *(kt->trapframe);
  ...
  /*
   ...
   maybe we should need to copy the context too!
  */
  ...
  pid = np->pid;
  release(&np->kthread[0].thread_lock); // added by shaun
...
{
  int intena;
  // struct proc *p = myproc(); // we changed, all below was with proccess
  struct kthread *kt = mykthread();
  if (!holding(&kt->thread_lock))
    panic("sched kt->thread_lock");
  if (mycpu()->noff != 1)
    panic("sched locks");
  if (kt->thread_state == T_RUNNING)
    panic("sched running");
  if (intr_get())
    panic("sched interruptible");

  intena = mycpu()->intena;
  swtch(&kt->thread_context, &mycpu()->t_context); // fixme
  mycpu()->intena = intena;
}
...
void yield(void) //??
{
  struct kthread *kt = mykthread();
  acquire(&kt->thread_lock);
  kt->thread_state = T_RUNNABLE; // fixme
  sched();
  release(&kt->thread_lock);
}
...
{
  struct kthread *kt = mykthread(); // change to kthread
  ...
  acquire(&kt->thread_lock); // DOC: sleeplock1
  release(lk);

  // Go to sleep.
  kt->thread_chan = chan;
  kt->thread_state = T_SLEEPING; // fixme
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    for (kt = p->kthread; kt < &p->kthread[NKT]; kt++)
    {
      if (kt != mykthread())
      {
        ...
        acquire(&kt->thread_lock);
        if (kt->thread_state == T_SLEEPING && kt->thread_chan == chan)
        { // fixme
...
void setkilled(struct proc *p) //???? we think it should be with thread
{
  acquire(&p->proc_lock);
  p->killed = 1;
  release(&p->proc_lock);
